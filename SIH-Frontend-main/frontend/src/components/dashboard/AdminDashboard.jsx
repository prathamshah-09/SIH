  ...
  const AdminDashboard = () => {
    ...
    // robust detection for "dark-like" themes (midnight calm) and "ocean-like" (ocean breeze)
    ...
    // helper values for card & text classes:
    const cardBorderClass = isDarkLike ? 'border border-white/20' : (isOceanLike ? 'border border-gray-200' : 'border border-gray-200');
    const cardBgClass = isDarkLike ? 'bg-transparent' : (isOceanLike ? 'bg-white' : 'bg-white');
    const labelTextClass = isDarkLike ? 'text-white' : (isOceanLike ? 'text-black' : 'text-gray-900');
    const valueTextClass = isDarkLike ? 'text-white' : (isOceanLike ? 'text-black' : 'text-gray-900');
    const featureTextClass = isDarkLike ? 'text-white' : (isOceanLike ? 'text-black' : '');
        ...
        <CardContent className="chat-panel">
          <Tabs value={chatTab} onValueChange={setChatTab} className="chat-panel">
            <TabsList 
              ...
              style={isDarkLike ? { backgroundColor: 'rgb(30 41 59)' } : {}}
            >
            ...
            <TabsContent value="chat" className="chat-panel">
              <div
                ...
                className={`chat-messages border rounded-xl ${isDarkLike ? 'bg-slate-800' : `bg-gradient-to-br ${theme.colors.secondary}`}`}
              >
              ...
              <div className={`chat-input-bar ${isDarkLike ? 'bg-slate-800' : 'bg-slate-800'}`}>
                <div className="chat-input-inner">
                  <textarea
                    ...
                    className={`flex-1 p-2 sm:p-3 border rounded-xl focus:ring-2 focus:ring-cyan-500 resize-none text-sm sm:text-base ${isDarkLike ? 'bg-slate-700 text-white' : 'bg-white'}`}
                    rows={1}
            ...
            <TabsContent value="history" className={`flex-1 overflow-hidden ${isDarkLike ? 'bg-slate-800' : ''}`}>
    ...
    const renderOverview = () => (
      <div className="space-y-8 overflow-x-hidden">
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 lg:gap-6">
          ...
          ].map(({ key, value, icon: Icon, change, changeColor }) => (
            <Card
              key={key}
              className={`bg-gradient-to-r ${cardBgClass} ${cardBorderClass} hover:shadow-2xl transition-all duration-300 hover:scale-105 cursor-pointer group`}
            >
              <CardContent className="p-4 md:p-6 lg:p-8">
                <div>
                  <p className={`text-sm font-semibold opacity-70 mb-2 ${labelTextClass}`}>{t(key)}</p>
                  <p className={`text-3xl font-bold ${valueTextClass} group-hover:scale-105 transition-transform`}>{value}</p>
